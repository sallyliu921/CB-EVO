...
int Gia_ManChoiceStore(Abc_Frame_t* pAbc, Gia_Man_t * pGia)
{
    // TODO: timing constraints
    Gia_Man_t * pTemp = Gia_ManDup(pGia);
    if(pTemp == NULL)
    {
        printf("Error: Gia_ManDup failed\n");
        return 1;
    }

    Vec_Ptr_t * vStore = pAbc->vStore;
    Vec_PtrPush(vStore, pTemp);

    return 0;
}

Gia_Man_t* Gia_ManChoiceCompute(Abc_Frame_t* pAbc, Dch_Pars_t * pPars)
{
    extern Aig_Man_t * Cec_ComputeChoicesNew( Gia_Man_t * pGia, int nConfs, int fVerbose );
    extern Vec_Ptr_t * Gia_ManOrderPios( Aig_Man_t * p, Gia_Man_t * pOrder );
    Vec_Ptr_t * vStore = pAbc->vStore;
    if(Vec_PtrSize(vStore) != 3)
    {
        printf("Error: must be 3 stored graphs\n");
        return NULL;
    }

    int graphCount = Vec_PtrSize(vStore);
    ...
    for(i = graphCount - 1; i >= 0; i--)
    {
        Vec_PtrPush(vGias, Vec_PtrEntry(vStore, i));
    }
    pMiter = Gia_ManChoiceMiter( vGias );
    Vec_PtrFree( vGias );

    // compute choices
    pMan = Cec_ComputeChoicesNew( pMiter, pPars->nBTLimit, pPars->fVerbose );
    Gia_ManStop( pMiter );

    // reconstruct the network
    vPios = Gia_ManOrderPios( pMan, vStore->pArray[0] ); 
    pMan = Aig_ManDupDfsGuided( pTemp = pMan, vPios );
    Aig_ManStop( pTemp );
    Vec_PtrFree( vPios );
    ...
    Gia_Man_t * pGiaTemp;
    Vec_PtrForEachEntry( Gia_Man_t *, vStore, pGiaTemp, i )
